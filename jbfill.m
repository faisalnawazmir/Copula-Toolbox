...
if nargin<7 || isempty(transparency)
    transparency=.5;
end %default is to have a transparency of .5

if nargin<6 || isempty(add)
    add=1;
end     %default is to add to current plot
if nargin<5 || isempty(edge)
    edge='k';
end  %dfault edge color is black
if nargin<4 || isempty(color)
    color='b';
end %default color is blue
