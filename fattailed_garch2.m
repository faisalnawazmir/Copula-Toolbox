function [parameters, likelihood, stderrors, robustSE, ht, scores] = fattailed_garch2(data , p , q , errors, startingvals, options)
...
%           'SKEWSTUDENTST' - Hansen's (1994) skewed Student's t (added 22jun04: andrew patton)
%
%
...
if strcmp(errors,'NORMAL') | strcmp(errors,'STUDENTST') | strcmp(errors,'GED') | strcmp(errors,'SKEWSTUDENTST')
  ...
  elseif strcmp(errors,'GED') 
      errortype = 3;
  else strcmp(errors,'SKEWSTUDENTST') 
      errortype = 4;
  end
else
   error('error must be one of the four strings NORMAL, STUDENTST, GED, or SKEWSTUDENTST');
end
...
elseif strcmp(errors,'SKEWSTUDENTST')
   ...
   LB(end-1)=2.1;
   LB(end) = -0.99;
   UB = 1e2*ones(1,p+q+3);  % don't want to constrain the other parameters
   UB(end) = 0.99;
   [n,m]=size(sumA);
   sumA = [sumA';zeros(2,n)]';
else
...
[parameters, LLF, EXITFLAG, OUTPUT, LAMBDA, GRAD, HESSIAN] =  fmincon('fattailed_garchlikelihood2', startingvals ,sumA  , sumB ,[] , [] , LB , UB,[],options, data, p , q, errortype);
...
hess = hessian('fattailed_garchlikelihood2',parameters,data,p,q,errortype);
[likelihood, ht]=fattailed_garchlikelihood2(parameters,data,p,q,errortype);
...
if nargout > 4
   ...
   for i=1:length(parameters)
      ...
      [HOLDER, HOLDER1, indivlike] = fattailed_garchlikelihood2(hparameters,data,p,q,errortype);
