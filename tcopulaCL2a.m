function CLa = tcopulaCL2a(theta,data)
...
CLa = gammaln((nu+2)/2) + gammaln(nu/2) - 2*gammaln((nu+1)/2) - 0.5*log((1-rho^2));
CLa = CLa - (nu+2)/2*log(1+(x.^2 + y.^2 - 2*rho*x.*y)/(nu*(1-rho^2)));
CLa = CLa + (nu+1)/2*log(1+x.^2/nu) + (nu+1)/2*log(1+y.^2/nu);
CLa = -CLa;